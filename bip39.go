...
var (
	...
	// wordListMutex protects wordList and wordMap for concurrent access
	wordListMutex sync.RWMutex
)
...
func NewEntropy(bitSize int) ([]byte, error) {
	...
	if n != len(entropy) {
		return nil, fmt.Errorf("insufficient entropy: got %d bytes, expected %d", n, len(entropy))
	}
	return entropy, nil
}
...
func EntropyFromMnemonic(mnemonic string) ([]byte, error) {
	...
	for _, v := range mnemonicSlice {
		...
		b = b.Or(b, tmp.SetBytes(wordBytes[:]))
	}
...
func NewMnemonic(entropy []byte) (string, error) {
	...
	// Reusable big.Int for AND masking.
	word := big.NewInt(0)
...
func MnemonicToByteArray(mnemonic string, raw ...bool) ([]byte, error) {
	...
	index := big.NewInt(0)
	for _, v := range mnemonicSlice {
		index.SetInt64(int64(wordMap[v]))
	...
	// Validate that the checksum is correct using constant-time comparison
	// to prevent timing attacks.
	newChecksummedEntropyBytes := padByteSlice(addChecksum(rawEntropyBytes), fullByteSize)
	if subtle.ConstantTimeCompare(checksummedEntropyBytes, newChecksummedEntropyBytes) != 1 {
		return nil, ErrChecksumIncorrect
	}
